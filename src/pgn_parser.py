...
class PGNTreeNode:
    def __init__(self, board: chess.Board, move: Optional[chess.Move] = None, move_san: Optional[str] = None):
        ...
        self.move_san = move_san         # SAN notation for the move (computed from parent's board).
        self.children = []               # For main line, this will be a single-element list.
        self.annotation: Optional[str] = None  # Annotation text.
        self.headers = {}                # To store PGN headers (only on the root)
...
def parse_pgn_to_tree(pgn_file_path: str) -> PGNTreeNode:
    ...
    storing the SAN for each move and the original headers.
    """
    ...
    # Store original headers in the root.
    root.headers = game.headers.copy()
    current_node = root

    for move in game.mainline_moves():
        move_san = board.san(move)  # Compute SAN from parent's board.
        board.push(move)
